...
public class DashboardQueries : IDashboardQueries
{
        ...
        CancellationToken ct)
    {
        ...
        var sql = @"
            -- CURRENT PERIOD
            SELECT 
                (SELECT COUNT(1)::INT FROM ""Auctions"" WHERE ""CreatedOnUtc"" >= @CurrStart AND ""CreatedOnUtc"" < @CurrEnd) AS ""TotalAuctions"",
                (SELECT COUNT(1)::INT FROM ""Auctions"" WHERE ""Status"" = 2 AND ""CreatedOnUtc"" >= @CurrStart AND ""CreatedOnUtc"" < @CurrEnd) AS ""LiveAuctions"",
                (SELECT COUNT(1)::INT FROM ""Auctions"" WHERE ""EndTime"" > @UtcNow AND ""EndTime"" <= (@UtcNow + INTERVAL '24 hours')) AS ""EndingWithin24h"",
                (SELECT COUNT(1)::INT FROM ""Orders"" WHERE ""Status"" = 4 AND ""CreatedOnUtc"" >= @CurrStart AND ""CreatedOnUtc"" < @CurrEnd) AS ""CompletedOrders"",
                (SELECT COUNT(1)::INT FROM ""Disputes"" WHERE ""Status"" = 1 AND ""CreatedAtUtc"" >= @CurrStart AND ""CreatedAtUtc"" < @CurrEnd) AS ""OpenDisputes"";

            -- PREVIOUS PERIOD
            SELECT 
                (SELECT COUNT(1)::INT FROM ""Auctions"" WHERE ""CreatedOnUtc"" >= @PrevStart AND ""CreatedOnUtc"" < @PrevEnd) AS ""TotalAuctions"",
                (SELECT COUNT(1)::INT FROM ""Auctions"" WHERE ""Status"" = 2 AND ""CreatedOnUtc"" >= @PrevStart AND ""CreatedOnUtc"" < @PrevEnd) AS ""LiveAuctions"",
                -- For the previous period 'Ending within 24h', we compare to the snapshot time of the previous period
                (SELECT COUNT(1)::INT FROM ""Auctions"" WHERE ""EndTime"" > @PrevUtcNow AND ""EndTime"" <= (@PrevUtcNow + INTERVAL '24 hours')) AS ""EndingWithin24h"",
                (SELECT COUNT(1)::INT FROM ""Orders"" WHERE ""Status"" = 4 AND ""CreatedOnUtc"" >= @PrevStart AND ""CreatedOnUtc"" < @PrevEnd) AS ""CompletedOrders"",
                (SELECT COUNT(1)::INT FROM ""Disputes"" WHERE ""Status"" = 1 AND ""CreatedAtUtc"" >= @PrevStart AND ""CreatedAtUtc"" < @PrevEnd) AS ""OpenDisputes"";
        ";
 ...
 public async Task<AuctionActivityDataResult> GetActivityTrendsAsync(
        ...
        CancellationToken ct)
    {
        var sql = @"
            ...
            SELECT 
                (SELECT COUNT(1)::INT FROM ""Auctions"" WHERE ""CreatedOnUtc"" >= @CurrStart AND ""CreatedOnUtc"" < @CurrEnd) AS ""CurrTotalAuctions"",
                (SELECT COUNT(1)::INT FROM ""Auctions"" WHERE ""CreatedOnUtc"" >= @PrevStart AND ""CreatedOnUtc"" < @PrevEnd) AS ""PrevTotalAuctions"",
                
                (SELECT COUNT(1)::INT FROM ""Bids"" WHERE ""PlacedAtUtc"" >= @CurrStart AND ""PlacedAtUtc"" < @CurrEnd) AS ""CurrTotalBids"",
                (SELECT COUNT(1)::INT FROM ""Bids"" WHERE ""PlacedAtUtc"" >= @PrevStart AND ""PlacedAtUtc"" < @PrevEnd) AS ""PrevTotalBids"";
            ...
            SELECT 
                CAST(""PlacedAtUtc"" AS DATE)::timestamp AS ""DatePoint"",
                COUNT(1)::INT AS ""Count""
            FROM ""Bids""
