...
{
    public UserQueries(ISqlConnectionFactory sqlFactory, IAsyncPolicy resiliencePolicy, ILogger<UserQueries> logger) : base(sqlFactory, resiliencePolicy, logger) { }
    ...
    public async Task<Result<Address>> GetAddressByIdAsync(Guid userId, CancellationToken cancellationToken)
    {
        ...
        var addressDto = await ExecuteResilientAsync(async (connection, ct) =>
            await connection.QueryFirstOrDefaultAsync<AddressDto>(
                new CommandDefinition(sql, new { UserId = userId }, cancellationToken: ct)), cancellationToken);
    ...
    public async Task<Result<Email>> GetEmailByIdAsync(Guid userId, CancellationToken cancellationToken)
    {
        ...
        var email = await ExecuteResilientAsync(async (connection, ct) =>
    ...
    public async Task<ProfileSettingsResponse?> GetProfileSettings(UserId userId, CancellationToken cancellationToken)
    {
        ...
        return await ExecuteResilientAsync(async (connection, ct) =>
               await connection.QueryFirstOrDefaultAsync<ProfileSettingsResponse>(
                   new CommandDefinition(sql, new { UserId = userId.Value }, cancellationToken: ct)), cancellationToken);
    }
