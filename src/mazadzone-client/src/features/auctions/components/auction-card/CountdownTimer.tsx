...
export function CountdownTimer(props: CountdownTimerProps) {
  if (props.status === "Ended") {
    return <StaticEndedTimer variant={props.variant} className={props.className} />;
  }

  return <ActiveCountdownTimer {...props} />;
}

function StaticEndedTimer({
  variant,
  className,
}: {
  variant?: "default" | "large" | "minimal";
...
function ActiveCountdownTimer({
  ...
  useEffect(() => {
    if (isExpired && isMounted && auctionId) {
      void queryClient.invalidateQueries({ queryKey: auctionKeys.detail(auctionId) });
    }
  }, [isExpired, isMounted, queryClient, auctionId]);
